...
class streetfigher(gym.Env):
    def __init__(self):
        ...
        self.pyboy.set_emulation_speed(2)  # Max speed
    ...
    def compute_repetition_penalty(self, action_history, current_action):
        ...
        # Apply a scaled penalty after a threshold (e.g., 3+ times)
        if repeated >= 3:
            return -1 * (repeated - 2)  # e.g., -0.1 for 3 reps, -0.2 for 4
        return 0.0

    def compute_movement_diversity_reward(self, action_history):
        movement_keys = [0, 1, 2, 3]  # Up, Down, Left, Right
        attack_keys = [4, 5]          # A (Punch), B (Kick)
        ...
        if last_action in movement_keys:
            ...
            # Percorrer os últimos 3 elementos anteriores ao ataque
            ...
            #print(5 * len(unique_movements))
            return 5 * len(unique_movements)
    ...
    def step(self, action):
        ...
        if self.enemy_hp[0] > self.enemy_hp[1] and self.enemy_hp != 255 and self.current_enemy_hp != 0:
            ...
            self.reward = self.damage_dealt + self.compute_movement_diversity_reward(list(self.action_history)) 
        ...
        if self.score[0] == 2:
            ...
            self.reward = self.reward + (1000 *  self.fights_won)
        ...
        if self.reward == 0:
            if all(x == self.action_history[0] for x in self.action_history):
                pass
                #print(self.reward)
        ...
        self.reward = self.reward + self.compute_repetition_penalty(self.action_history,action)
